...
% =========================
\begin{abstract}
Sparse principal component analysis (SPCA) improves interpretability by inducing sparse loadings, but standard formulations often treat features as exchangeable and ignore known relational structure. In many applications, including genomics, sensor networks, and finance, features are coupled by an underlying graph. We study a network-constrained SPCA formulation that combines variance maximization with an $\ell_1$ sparsity penalty and a graph Laplacian smoothness regularizer. For the single-component problem with a unit $\ell_2$ ball constraint, we derive an exact proximal-gradient method for a nonconvex smooth term plus a convex nonsmooth term, including the ball indicator. The resulting proximal map has the closed form soft-thresholding followed by radial projection. Under Lipschitz continuity of the smooth gradient, the method converges to critical points in the standard nonconvex composite sense. We also explain why multi-component extensions require manifold-aware proximal methods. Empirically, the method should be viewed as conditionally useful: structural gains depend on graph quality and can deteriorate under misspecification.
\end{abstract}
...
This paper studies \emph{network-constrained sparse PCA}, which augments SPCA with a graph Laplacian regularizer to encourage loadings that are simultaneously sparse and smooth over a feature graph. The Laplacian term penalizes local variation across graph edges and therefore favors coherent selections when the graph is informative. Our focus is twofold: (i) a single-component formulation with a unit $\ell_2$ ball constraint and an exact proximal-gradient algorithm, and (ii) a scoped discussion of how the optimization landscape changes in multi-component extensions with orthogonality constraints.
...
\begin{itemize}[leftmargin=*]
  ...
  \item We empirically evaluate structured feature selection on synthetic graph families and identify the regimes in which graph regularization helps or hurts support recovery.
\end{itemize}
...
\subsection{\texorpdfstring{Practical Accelerated Variant\\ (MASPG-CAR)}{Practical Accelerated Variant (MASPG-CAR)}}
Algorithm~\ref{alg:pg} is a clean baseline and the basis of our formal convergence result. For practical large-scale tuning and repeated solves across many regularization pairs, we recommend an enhanced variant that preserves the same exact composite proximal map while often improving wall-clock efficiency in practice.

\paragraph{Design principles.}
The practical variant combines: (i) spectral (Barzilai--Borwein) step-size initialization, (ii) monotone inertial extrapolation with restart, (iii) continuation or warm starts over $(\lambda_1,\lambda_2)$, and (iv) optional active-set restricted refinement once support stabilizes. These modifications are standard acceleration heuristics in first-order optimization, but we present them as a \emph{practical} solver variant rather than as a replacement for the theorem-backed baseline.
...
\begin{equation}
\begin{aligned}
f(w_{\mathrm{trial}}^{k+1}) \le\; & f(y^k)
 + \langle \nabla f(y^k), w_{\mathrm{trial}}^{k+1}-y^k\rangle \\
& + \frac{1}{2\eta_k}\|w_{\mathrm{trial}}^{k+1}-y^k\|_2^2,
\end{aligned}
\end{equation}
...
In contrast, multi-component formulations with $V^\top V=I_k$ add a nonconvex manifold constraint. A straightforward ``threshold then orthogonalize'' pipeline is unstable for sparsity because orthogonalization mixes coordinates and can re-densify loadings.

Manifold proximal-gradient (ManPG-type) methods address this by solving a proximal subproblem in the tangent space of the Stiefel manifold and then retracting back to the manifold \cite{chen2020manpg,chen2020amanpg}. The graph Laplacian term contributes the smooth gradient term $2\lambda_2 L V$, so it fits naturally into the smooth part of such methods.
...
\begin{table}[t]
\centering
\caption{Aggregate synthetic benchmark results from the current audit run. Values are means across chain, grid, random geometric graph, and SBM families.}
...
\paragraph{Current evidence from the repository.}
In the base synthetic benchmark, $\ell_1$-SPCA achieves the highest mean support F1 among the core methods currently reported. The network-constrained variants remain close in F1 but do not exceed the unstructured sparse baseline in the current aggregate audit. Under graph misspecification, the gap in favor of unstructured $\ell_1$-SPCA widens further. For optimization, MASPG-CAR reduces average iterations relative to baseline PG, and continuation provides the clearest measured runtime gain over naive grid traversal.

\paragraph{Optimization ablation summary.}
The current ablation artifacts show that MASPG-CAR reduces the mean iteration count from $9.5$ to $6.0$ relative to baseline PG, with runtime changing from $0.0094$ s to $0.0090$ s in the audited setting. Continuation or warm-start traversal reduces total path iterations from $136$ to $91$, corresponding to a measured $1.43\times$ speedup over naive grid traversal.

\paragraph{Expected qualitative trends.}
When the graph prior is informative, increasing $\lambda_2$ should reduce Laplacian energy and improve support coherence (e.g., larger connected-component ratios) at a fixed sparsity level. Excessive graph regularization may over-smooth the loadings and reduce support precision or explained variance. Under graph misspecification, unstructured $\ell_1$-SPCA can match or exceed network-constrained variants on support F1, so the useful operating regime is a Pareto region rather than a single universally optimal parameter pair.
