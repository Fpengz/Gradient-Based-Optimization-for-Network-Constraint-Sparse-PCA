...
DEFAULT_TABLE = ROOT / "results" / "phase2" / "blockB" / "metrics_summary.csv"
DEFAULT_OUT = ROOT / "figures" / "phase2" / "blockB" / "blockB_sanity_lambda2_sweep.png"


def write_blockB_sanity_plot(df: pd.DataFrame, out_path: Path) -> None:
    base = df[(df["graph_family"] == "grid") & (df["method"] == "Proposed")]
    clean = base[
    ...
    if clean.empty or delete.empty or rewire.empty:
        raise ValueError("Missing clean/delete/rewire data for blockB sanity plot")
...
def main() -> None:
    ...
    write_blockB_sanity_plot(df, DEFAULT_OUT)
