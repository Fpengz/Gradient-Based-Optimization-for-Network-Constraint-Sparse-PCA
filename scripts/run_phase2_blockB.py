...
DEFAULT_MANIFEST = ROOT / "configs" / "phase2" / "blockB" / "manifest.csv"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 2 Block B configs")
    parser.add_argument(
        "--manifest",
        default=str(DEFAULT_MANIFEST),
        help="Path to blockB manifest.csv",
    )
    parser.add_argument(
        "--sanity",
        action="store_true",
