...
\begin{itemize}[leftmargin=*]
    \item We study an executed synthetic PCA protocol with four methods only: PCA, A-ManPG, SparseNoGraph, and the Proposed method.
    \item We restrict the empirical scope to chain and SBM graphs, five random seeds ($0$--$4$), and shared evaluation metrics only.
    \item We distinguish SparseNoGraph as an in-family ablation with $\lambda_2=0$ from A-ManPG as the minimal external sparse baseline without graph coupling.
\end{itemize}
...
The step size for the $A$-step is fixed at the experimental value $\eta_A=0.05$. The $B$-step uses
\[
\eta_B = \frac{1}{2\lambda_2 \|L\|_2 + \rho},
\]
which matches the Lipschitz constant of the smooth part in the Euclidean update.

\subsection{Stopping Criteria}
The outer loop stops when any of the following is satisfied:
\begin{itemize}[leftmargin=*]
    \item relative objective change falls below a tolerance,
    \item the coupling gap $\|A-B\|_F$ falls below a tolerance,
    \item the orthogonality defect $\|A^\top A - I_r\|_F$ falls below a tolerance,
    \item the maximum iteration count is reached.
\end{itemize}
...
We evaluate only the executed synthetic study. The comparison set is limited to PCA, A-ManPG, SparseNoGraph, and the Proposed method. SparseNoGraph is the in-family ablation with $\lambda_2=0$, while A-ManPG is the minimal external sparse orthogonal baseline without graph coupling. The graph families are chain and SBM, and all main results use seeds $0$--$4$. Shared evaluation metrics only are reported: support recovery, graph smoothness on the true graph, and shared explained variance. The explained-variance quantity is an evaluation metric computed after QR-based orthonormalization, not a training objective for any method.
