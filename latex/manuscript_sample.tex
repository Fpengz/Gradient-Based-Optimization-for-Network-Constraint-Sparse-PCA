...
\begin{itemize}[leftmargin=*]
    ...
    \item In a graph-aligned chain setting with disjoint contiguous supports and scattered high-variance decoys, a narrow low-$\lambda_2$ band yields strong smoothness gains with no clear support-F1 penalty relative to SparseNoGraph and exceeds A-ManPG across metrics in the high-decoy regime.
...
Figure~\ref{fig:graph-aligned-high} summarizes the high-decoy graph-aligned setting. A narrow low-$\lambda_2$ band around $\lambda_2 \in [0.05, 0.10]$ yields strong smoothness reductions relative to SparseNoGraph with no clear support-F1 penalty, while the Proposed method exceeds A-ManPG on recovery, smoothness, and connectivity. Table~\ref{tab:graph-aligned-summary} reports a representative $\lambda_2=0.05$ setting (20 seeds). Figure~\ref{fig:graph-aligned-medium} provides the medium-decoy supporting view, which follows a similar but weaker pattern.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{high_decoy_band.png}
\caption{High-decoy graph-aligned setting (20 seeds, mean $\pm$ SEM). In the high-decoy regime, the proposed method shows a narrow low-$\lambda_2$ band ($[0.05, 0.10]$, shaded) with strong reductions in graph smoothness norm relative to SparseNoGraph and no clear support-F1 penalty. Across the same band, the proposed method exceeds A-ManPG on support F1, smoothness, and connectivity. Larger $\lambda_2$ values oversmooth and eventually reduce recovery.}
...
\caption{Medium-decoy graph-aligned setting (20 seeds, mean $\pm$ SEM). The medium-decoy regime shows a similar but weaker pattern: low-to-moderate graph regularization improves smoothness while keeping support recovery near the sparse non-graph baseline over a narrow $\lambda_2$ range. The benefit is more modest than in the high-decoy setting, indicating that the usefulness of graph regularization depends on distractor strength.}
...
\caption{Difference summary relative to SparseNoGraph (20 seeds, mean $\pm$ SEM). Proposed minus SparseNoGraph across decoy intensities. In the high-decoy setting, a narrow low-$\lambda_2$ region yields strong smoothness gains with no clear F1 penalty; outside this region, larger $\lambda_2$ values reduce recovery. Medium-decoy behavior is directionally similar but weaker.}
