...
\begin{itemize}[leftmargin=*]
    ...
    \item In a graph-aligned chain setting with disjoint contiguous supports and scattered high-variance decoys, we identify a stable low-to-moderate ($\lambda_2$) operating regime with strong smoothness gains and no detectable recovery degradation relative to SparseNoGraph; this regime persists across tested $\lambda_1$ and $\rho$ settings, though its width is tuning-dependent.
...
Figure~\ref{fig:graph-aligned-high} summarizes the high-decoy graph-aligned setting. Robustness sweeps over $\lambda_1$ and $\rho$ show that the low-to-moderate ($\lambda_2$) operating regime persists across all 9 tested configurations. Under the default configuration this regime is centered in the previously identified low-($\lambda_2$) region, while the interval broadens or narrows with tuning; in particular, it narrows for $\lambda_1=0.05$ and $\rho\in\{5,10\}$. Table~\ref{tab:graph-aligned-summary} reports a representative $\lambda_2=0.05$ setting (20 seeds). Figure~\ref{fig:graph-aligned-medium} provides the medium-decoy supporting view, which follows a similar but weaker pattern.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{high_decoy_band.png}
\caption{High-decoy graph-aligned setting (20 seeds, mean $\pm$ SEM). In the high-decoy regime, the proposed method shows a low-to-moderate ($\lambda_2$) operating regime with strong reductions in graph smoothness norm relative to SparseNoGraph and no clear support F1 penalty. Across the same regime, the proposed method exceeds A-ManPG on support F1, graph smoothness norm, and connectivity. Larger $\lambda_2$ values oversmooth and eventually reduce recovery.}
...
A small robustness sweep over $\lambda_1$ and $\rho$ shows that the operating regime is not confined to a single default configuration: it appears across all tested settings, with broader intervals in most cases and narrower intervals when $\lambda_1=0.05$ and $\rho\in\{5,10\}$.
