...
% =========================
\begin{abstract}
We study sparse, graph-smooth, explicitly orthogonal multi-component PCA under controlled graph misspecification. Using a split-variable solver as a testbed, we map how weak versus strong regularization behaves on fragile (chains) and redundant (grids) graphs. At low $\lambda_2$, performance is robust to misspecification; at higher $\lambda_2$, chains degrade under rewiring while grids remain largely invariant. We document an illusory smoothness effect: smoothness under the imposed (possibly corrupted) graph can improve even as support recovery worsens. We also show a graph-aligned regime in which graph regularization genuinely helps, and include a small TCGA-BRCA sanity check. Our claims are intentionally modest: graph regularization can help in aligned settings, but its reliability depends on regularization strength and topology, and smoothness under the imposed graph can be misleading.
\end{abstract}
...
At $\lambda_2=0.5$ and $\alpha=0.2$ on the chain, rewiring reduces the imposed-graph Laplacian energy from $1.916$ to $1.599$ ($\Delta=-0.317$) while true-graph smoothness worsens (mean increases by $0.323$) and support F1 drops by $0.058$ (mean over seeds $0$--$2$), illustrating that smoothness under a corrupted prior can improve even as fidelity to the true graph and recovery degrade.
...
Appendix~\ref{app:topology} reports SBM and kNN misspecification results that follow the same topology-dependent pattern.
\begin{figure}[t]
\centering
...
We run a $\lambda_2$ sweep on TCGA-BRCA expression with a STRING PPI graph using three random initializations and report smoothness and shared explained variance (Figure~\ref{fig:realdata}). We observe that moderate $\lambda_2$ yields stable variance--smoothness tradeoffs, while larger $\lambda_2$ leads to more aggressive smoothing with diminishing variance gains, consistent with the synthetic regime behavior.
\begin{figure}[t]
\centering
...
The mechanism paragraph is simple. Chains depend on precise adjacency: rewiring breaks the local relationships that Laplacian smoothness is meant to encode. Grids have redundant paths; redundancy flattens the effect of perturbations and reduces sensitivity to misspecification.

The main limitation of the current evidence is its synthetic emphasis and narrow topology coverage in the main text. We include a small TCGA-BRCA sanity check and move broader topology and real-data evidence to the appendix. These results demonstrate that structural priors can help, but they also show that regularization strength must be tuned to topology and that smoothness under the imposed graph should not be conflated with structural fidelity.

\section{Conclusion}
\label{sec:conclusion}

Graph regularization is not universally helpful. Its reliability depends on regularization strength and topology, and smoothness under the imposed graph can be misleading under misspecification. In aligned settings, however, graph priors can improve structural coherence without sacrificing recovery. These findings support a modest, defensible claim: use graph regularization with topology-aware tuning and interpret smoothness gains cautiously, especially when the graph is sparse or known to be noisy.
...
\caption{Change relative to the clean graph at a representative high-regularization setting ($\lambda_2 = 0.5, \alpha = 0.4$). Chain shows clear degradation, whereas grid, SBM, and kNN exhibit little degradation or slight improvement. Bars show mean $\Delta$F1 over seeds; error bars indicate standard error.}
