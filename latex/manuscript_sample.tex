...
We evaluate only the executed synthetic study. The comparison set includes PCA, A-ManPG, GraphOnlyPCA ($\lambda_1=0$), a non-manifold graph+sparsity baseline (GraphSparseNoOrth), SparseNoGraph, and the Proposed method. SparseNoGraph is the in-family ablation with $\lambda_2=0$, while A-ManPG is the minimal external sparse orthogonal baseline without graph coupling. The graph families are chain and SBM. Main robustness results use seeds $0$--$4$, while sweep experiments and the graph-aligned setting use seeds $0$--$2$. Shared evaluation metrics only are reported: support recovery, graph smoothness on the true graph, shared explained variance, and support connectivity. For sparse baselines, we select $\lambda_1$ from a small grid to match the sparsity level (nnz) of the Proposed method for primary comparisons.
