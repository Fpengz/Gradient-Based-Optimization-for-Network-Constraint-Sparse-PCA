...
Sweeping $\lambda_2 \in \{0.0, 0.05, 0.1, 0.2, 0.5\}$ reveals an overall smoothness--recovery tradeoff. Light graph regularization improves smoothness with minimal loss in support F1, while larger $\lambda_2$ values steadily decrease support F1 and increase support density. Shared explained variance declines moderately as $\lambda_2$ grows. This sweep indicates that $\lambda_2$ must be tuned to graph topology; values that are benign on chains can oversmooth on SBM.
\begin{figure}[t]
\centering
