...
We run a $\lambda_2$ sweep on TCGA-BRCA expression with a STRING PPI graph using three random initializations and report smoothness and shared explained variance (Figure~\ref{fig:realdata}). We observe that moderate $\lambda_2$ yields stable variance--smoothness tradeoffs, while larger $\lambda_2$ leads to more aggressive smoothing with diminishing variance gains, consistent with the synthetic regime behavior. A subsampling robustness sweep (80\% subsamples, 3 seeds; Figure~\ref{fig:realdata-robust}) shows the same qualitative pattern across random subsamples. For real data we show the proposed method only, since support recovery metrics are not defined without ground truth.
...
\caption{Multi-component sweep on SBM graphs with connected supports. Mean per-component F1 and orthogonality error are reported for $r \in \{2,3,5\}$; recovery decreases at higher $\lambda_2$ while orthogonality remains stable.}
...
\caption{Multi-component sweep on SBM graphs with fragmented supports. Mean per-component F1 and orthogonality error are reported for $r \in \{2,3,5\}$; recovery declines more sharply as $\lambda_2$ increases while orthogonality remains stable.}
...
\caption{Multi-component sweep on SBM graphs with cross-community supports. Mean per-component F1 and orthogonality error are reported for $r \in \{2,3,5\}$; strong regularization amplifies mismatch, yielding lower recovery at higher $\lambda_2$ despite stable orthogonality.}
...
\caption{Real-data robustness sweep on TCGA-BRCA under random 80\% subsampling (3 seeds). The variance--smoothness tradeoff remains consistent across subsamples, with diminishing variance gains at higher $\lambda_2$.}
