...
We evaluate only the executed synthetic study. The comparison set is limited to PCA, A-ManPG, SparseNoGraph, and the Proposed method. SparseNoGraph is the in-family ablation with $\lambda_2=0$, while A-ManPG is the minimal external sparse orthogonal baseline without graph coupling. The graph families are chain and SBM. Main robustness results use seeds $0$--$4$, while sweep experiments and the graph-aligned setting use seeds $0$--$2$. Shared evaluation metrics only are reported: support recovery, graph smoothness on the true graph, shared explained variance, and support connectivity.
...
A-ManPG & 0.861 $\pm$ 0.016 & 2.049 $\pm$ 0.471 & 0.286 $\pm$ 0.014 \\
SparseNoGraph & 0.934 $\pm$ 0.047 & 2.049 $\pm$ 0.457 & 0.302 $\pm$ 0.039 \\
