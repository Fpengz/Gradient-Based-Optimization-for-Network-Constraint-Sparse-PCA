...
Our study is guided by two central questions. First, when does feature-graph regularization improve the recovery of interpretable sparse components? Second, what tradeoff does graph regularization induce between explained variance, graph consistency, and computational cost? These questions are important because graph-based regularization can help when the prior graph is informative, but may oversmooth the loadings or reduce variance capture when the graph is misspecified. Accordingly, the paper emphasizes not only model design and optimization, but also the empirical circumstances under which graph priors are beneficial.

\paragraph{Contributions.}
The main contributions of this work are as follows.
\begin{itemize}[leftmargin=*]
\item We define a PCA-specific split-variable objective that couples explicit orthogonality with sparse graph-smooth loadings.
    \item We specify a single alternating solver with explicit update rules for both the manifold and proximal steps.
    \item We lay out a synthetic evaluation protocol with a complete baseline matrix and mandatory ablations that test the contribution of graph regularization, sparsity, and joint orthogonality.
\end{itemize}
...
We defer alternative sparsity structures to later work to keep the paper-1 formulation fixed to entrywise $\ell_1$ on $B$.
...
\subsection{Assumptions and Roadmap}
We assume the following:
\begin{itemize}[leftmargin=*]
    \item $L \succeq 0$ and $\lambda_1,\lambda_2,\rho > 0$.
    \item Step sizes satisfy standard Lipschitz or backtracking conditions for the proximal-gradient update.
    \item Iterates remain in a bounded level set due to the quadratic coupling term.
\end{itemize}
Planned analysis targets for paper 1:
\begin{itemize}[leftmargin=*]
    \item lower bounded objective,
    \item sufficient decrease or monotone descent of the objective,
...
Synthetic experiments are essential because the ground-truth support and latent subspace are known.
\paragraph{Graph families.}
Use the following families:
\begin{itemize}[leftmargin=*]
    \item chain graph,
    \item 2D grid graph,
    \item stochastic block model with three blocks (intra-p $0.2$, inter-p $0.02$),
    \item random geometric graph (optional if budget allows).
\end{itemize}

\paragraph{Ground-truth supports.}
Construct sparse supports that are:
\begin{itemize}[leftmargin=*]
    \item connected,
    \item nearly connected (one edge removed),
    \item disconnected (two subgraphs),
    \item overlap regimes: disjoint supports and 50\% shared support.
\end{itemize}

\paragraph{Parameter sweeps.}
Use the fixed ranges below:
\begin{itemize}[leftmargin=*]
    \item $n \in \{200, 500, 1000\}$,
    \item $p \in \{200, 500, 1000\}$,
    \item $r \in \{3, 5\}$,
    \item sparsity $s \in \{10, 20, 40\}$,
    \item SNR $\in \{0.5, 1, 2\}$,
    \item graph corruption $\in \{0, 0.1, 0.3\}$,
    \item $\lambda_1$ and $\lambda_2$ on a $5 \times 5$ log grid.
\end{itemize}
\paragraph{Replicates.}
Run 20 replicates per configuration.

\subsection{Baselines}
Because graph+sparsity PCA already exists in the literature, comparisons against only PCA and plain SPCA are not enough. At minimum, include:
\begin{enumerate}[leftmargin=*]
    \item PCA,
    \item standard sparse PCA,
    \item a joint-orthogonal sparse PCA baseline,
    \item a graph-aware PCA baseline,
    \item a graph+sparsity PCA baseline,
    \item your method without graph regularization,
    \item your method without sparsity,
    \item your method with deflation instead of joint extraction.
\end{enumerate}

\subsection{Metrics}
Report metrics that match the claims:
\paragraph{Statistical metrics.}
Cumulative explained variance, reconstruction error, subspace distance to truth, orthogonality error.
\paragraph{Support metrics.}
Support size, precision, recall, F1, Jaccard overlap.
\paragraph{Graph metrics.}
Laplacian energy $\tr(B^\top L B)$, connected-component count or ratio on support, support smoothness.
\paragraph{Optimization metrics.}
Runtime, memory, outer iterations, inner iterations.
\paragraph{Robustness metrics.}
Sensitivity to graph corruption and bootstrap support stability.

\section{Results}
\label{sec:results}

\paragraph{Overview.}
We report synthetic results using shared evaluation metrics across all methods: union support F1, graph smoothness norm on the true graph, and a shared explained-variance measure computed from orthonormalized loadings. Shared explained variance is an evaluation metric and is not the training objective for any of the methods. We do not compare raw objective values across methods because they optimize different objectives. Results are presented for chain and SBM graphs across five seeds (0--4), and for a single-seed SBM sweep over $\lambda_2$. The comparison set includes PCA, A-ManPG, SparseNoGraph, and the Proposed method.

\subsection{Chain Robustness (Seeds 0--4)}
Across seeds, PCA is consistently worst on support recovery. A-ManPG improves substantially over PCA but remains below the sparse baselines on mean support F1. SparseNoGraph achieves the highest mean support F1, while the Proposed method attains slightly lower F1 but achieves a clearly lower graph smoothness norm on the true chain graph. Shared explained variance is similar across the non-PCA methods and lower than PCA, as expected for sparse solutions. This pattern indicates that on chain graphs, graph regularization improves structural coherence while remaining competitive on recovery.
\begin{table}[t]
\centering
\scriptsize
...
\subsection{SBM \texorpdfstring{$\lambda_2$}{lambda2} Sweep (Seed 2)}
Sweeping $\lambda_2 \in \{0.0, 0.05, 0.1, 0.2, 0.5\}$ reveals a monotone smoothness--recovery tradeoff. Light graph regularization improves smoothness with minimal loss in support F1, while larger $\lambda_2$ values steadily decrease support F1 and increase support density. Shared explained variance declines moderately as $\lambda_2$ grows. This sweep indicates that $\lambda_2$ must be tuned to graph topology; values that are benign on chains can oversmooth on SBM.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/sbm_lambda2_sweep_panel.png}
\caption{SBM lambda2 sweep (seed 2). Light regularization improves smoothness with limited recovery loss, while stronger regularization oversmooths and degrades support recovery.}
...
The main limitation of the current evidence is that it remains synthetic and focuses on two graph families with a single data generator. This is appropriate for Paper 1, whose goal is to establish a disciplined method and a clear empirical tradeoff. The results already demonstrate that structural priors can improve graph coherence, but they also show that regularization strength must be tuned to topology. This is a valid and useful contribution at the current scope.

\section{Conclusion}
\label{sec:conclusion}

We present synthetic evidence that graph-regularized sparse orthogonal PCA improves structural coherence while inducing a topology-dependent tradeoff with support recovery. On chain graphs, the proposed method is competitive on recovery and yields smoother supports; on SBM graphs, stronger graph regularization improves smoothness but reduces recovery accuracy. These findings highlight the need for topology-aware $\lambda_2$ tuning and support a modest, defensible Paper-1 claim: graph regularization can improve structural coherence, but its effect on recovery depends on graph topology and regularization strength.

\appendix

\section{Proof Sketches and Deferred Arguments}
\label{app:proofs}
This appendix is a placeholder for proof details, technical lemmas, and auxiliary propositions.

\section{Additional Experimental Details}
\label{app:exp}
This appendix is a placeholder for hyperparameter grids, implementation details, dataset preprocessing, and ablation tables.
