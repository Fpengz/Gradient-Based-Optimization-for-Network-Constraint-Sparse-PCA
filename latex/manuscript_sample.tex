...
We evaluate only the executed synthetic study. The comparison set is limited to PCA, A-ManPG, SparseNoGraph, and the Proposed method. SparseNoGraph is the in-family ablation with $\lambda_2=0$, while A-ManPG is the minimal external sparse orthogonal baseline without graph coupling. The graph families are chain and SBM. Main robustness results use seeds $0$--$4$, sweep experiments use seeds $0$--$2$, and the graph-aligned decoy study uses seeds $0$--$19$. Shared evaluation metrics only are reported: support F1, graph smoothness norm on the true graph, shared explained variance, and support connectivity.
