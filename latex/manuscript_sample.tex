...
% =========================
\begin{abstract}
We study sparse, graph-smooth, explicitly orthogonal multi-component PCA through a split-variable formulation that couples an orthogonal loading matrix with a sparse graph-regularized proxy. We propose a concrete alternating solver consisting of a Stiefel manifold update for the orthogonal factor and a proximal-gradient update for the sparse graph-smooth factor. We evaluate the method on synthetic graph families using shared evaluation metrics (support recovery, graph smoothness on the true graph, and shared explained variance). In our synthetic experiments, graph regularization yields smoother solutions on the true graph, while its effect on support recovery is topology-dependent: on chain graphs the method is competitive on recovery and smoother on the true graph, whereas on SBM graphs stronger regularization induces a recovery--smoothness tradeoff and light regularization can improve smoothness with limited recovery loss. We also include a graph-aligned chain setting with disjoint contiguous supports and scattered high-variance decoys, where a low-to-moderate $\\lambda_2$ regime (approximately $0.04$--$0.11$) yields strong smoothness gains with no clear support-F1 degradation relative to SparseNoGraph, while exceeding A-ManPG across recovery, smoothness, and connectivity in the high-decoy regime. These benefits are tuning-sensitive, and stronger regularization eventually oversmooths and reduces recovery. The study is synthetic-first and limited to chain and SBM topologies; broader theoretical guarantees and real-data validation remain future work.
\end{abstract}
...
\begin{itemize}[leftmargin=*]
    ...
    \item In a graph-aligned chain setting with disjoint contiguous supports and scattered high-variance decoys, a low-to-moderate $\\lambda_2$ regime (roughly $0.04$--$0.11$) yields strong smoothness gains with no clear support-F1 degradation relative to SparseNoGraph and exceeds A-ManPG across metrics in the high-decoy regime.
...
We evaluate only the executed synthetic study. The comparison set is limited to PCA, A-ManPG, SparseNoGraph, and the Proposed method. SparseNoGraph is the in-family ablation with $\lambda_2=0$, while A-ManPG is the minimal external sparse orthogonal baseline without graph coupling. The graph families are chain and SBM. Main robustness results use seeds $0$--$4$, sweep experiments use seeds $0$--$2$, and the graph-aligned decoy study uses seeds $0$--$19$. Shared evaluation metrics only are reported: support recovery, graph smoothness on the true graph, shared explained variance, and support connectivity.
...
Figure~\ref{fig:graph-aligned-high} summarizes the high-decoy graph-aligned setting. A low-to-moderate $\\lambda_2$ regime (approximately $0.04$--$0.11$) yields strong smoothness reductions relative to SparseNoGraph with no clear support-F1 degradation, while the Proposed method exceeds A-ManPG on recovery, smoothness, and connectivity. This regime is gradual rather than sharply localized: edge comparisons at $\\lambda_2\\in\\{0.04,0.05,0.10,0.11,0.12\\}$ show support-F1 confidence intervals that continue to include zero through $\\lambda_2=0.12$, whereas smoothness gains increase monotonically across the sweep. Table~\ref{tab:graph-aligned-summary} reports a representative $\\lambda_2=0.05$ setting (20 seeds). Figure~\ref{fig:graph-aligned-medium} provides the medium-decoy supporting view, which follows a similar but weaker pattern.
...
\caption{High-decoy graph-aligned setting (20 seeds, mean $\pm$ SEM). In the high-decoy regime, the proposed method shows a low-to-moderate $\\lambda_2$ regime (approximately $0.04$--$0.11$, shaded) with strong reductions in graph smoothness norm relative to SparseNoGraph and no clear support-F1 degradation. Across the same regime, the proposed method exceeds A-ManPG on support F1, smoothness, and connectivity. Larger $\\lambda_2$ values oversmooth and eventually reduce recovery.}
...
\caption{Medium-decoy graph-aligned setting (20 seeds, mean $\pm$ SEM). The medium-decoy regime shows a similar but weaker pattern: low-to-moderate graph regularization improves smoothness while keeping support recovery near the sparse non-graph baseline over a tuning-sensitive $\\lambda_2$ range. The benefit is more modest than in the high-decoy setting, indicating that the usefulness of graph regularization depends on distractor strength.}
...
We present synthetic evidence that graph-regularized sparse orthogonal PCA improves structural coherence while inducing a topology-dependent tradeoff with support recovery. On chain graphs, the proposed method is competitive on recovery and yields smoother supports; on SBM graphs, stronger graph regularization improves smoothness but reduces recovery accuracy. In a graph-aligned chain setting with disjoint contiguous supports and scattered decoys, a low-to-moderate $\\lambda_2$ regime (approximately $0.04$--$0.11$) yields strong smoothness gains with no clear support-F1 degradation relative to SparseNoGraph while exceeding A-ManPG across metrics in the high-decoy regime. These findings highlight the need for topology-aware, tuning-sensitive $\\lambda_2$ selection and support a modest, defensible claim: graph regularization improves structural coherence, and its recovery impact depends on topology and regularization strength.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Per-$\\lambda_2$ inference (Holm/BH):} Adjusted tests show statistically significant improvements in normalized graph smoothness for all $\\lambda_2$, while no adjusted comparison detects a support-F1 difference versus SparseNoGraph within the low-to-moderate regime. (See \texttt{results/trackB/phase1\_5/phase15\_per\_lambda\_holm\_bh.csv}.)
    \item \textbf{Band-edge diagnostics:} Local comparisons at $\\lambda_2\\in\\{0.04, 0.05, 0.10, 0.11, 0.12\\}$ indicate a gradual transition: confidence intervals for $\\Delta$F1 include zero through $\\lambda_2\\approx 0.12$, while smoothness gains increase monotonically. (See \texttt{results/trackB/phase1\_5/phase15\_band\_edge\_summary.csv}.)
    \item \textbf{Component-level checks:} Component-wise metrics do not indicate hidden degradation within the regime; worst-component F1 does not collapse and is slightly improved at $\\lambda_2=0.04$--$0.05$. (See \texttt{results/trackB/phase1\_5/phase15\_component\_summary.csv}.)
...
\caption{Difference summary relative to SparseNoGraph (20 seeds, mean $\pm$ SEM). Proposed minus SparseNoGraph across decoy intensities. In the high-decoy setting, a low-to-moderate $\\lambda_2$ regime yields strong smoothness gains with no clear F1 degradation; outside this regime, larger $\\lambda_2$ values reduce recovery. Medium-decoy behavior is directionally similar but weaker.}
