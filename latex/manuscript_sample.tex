...
A key takeaway is that the effect of graph regularization depends critically on graph topology. Sparse, fragile graphs such as chains are highly sensitive to misspecification, while redundant graphs such as grids, SBM, and kNN are more robust and can even alleviate oversmoothing in certain regimes. This suggests that the usefulness of graph priors in sparse PCA is not universal, but depends on structural redundancy in the underlying graph. Understanding this distinction is essential when applying graph-regularized methods in practice.
