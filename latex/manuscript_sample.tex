...
\usepackage{graphicx}
\graphicspath{{../figures/}{../figures/phase2/}{../figures/phase2/compare/}{../figures/phase2/realdata/}{../figures/realdata/}}
...
% Title
% =========================
\title{\texorpdfstring{Topology-Dependent Reliability of Graph Regularization under Misspecification in Sparse PCA}{Topology-Dependent Reliability of Graph Regularization under Misspecification in Sparse PCA}}
\author{Wang Zhoufu}
\date{}
...
% =========================
\begin{abstract}
We study sparse, graph-smooth, explicitly orthogonal multi-component PCA under controlled graph misspecification. Using a concrete split-variable solver as a controlled testbed, we find a regime split: weak regularization is robust across priors, while stronger regularization induces topology-dependent sensitivity---chains degrade under rewiring whereas grids remain largely invariant. We document an illusory smoothness effect: smoothness under the imposed (possibly corrupted) graph can improve even as support recovery worsens. We also identify a graph-aligned regime in which regularization helps, and include a small TCGA-BRCA sanity check. Our claims are intentionally modest: graph regularization can help in aligned settings, but its reliability depends on regularization strength and topology, and smoothness under the imposed graph can be misleading.
\end{abstract}
...
Our study is guided by a single question: when is graph regularization reliable under misspecification? We take a regime view in which the effect of graph regularization depends jointly on $\lambda_2$ and topology: weak regularization behaves robustly across priors, while stronger regularization can amplify errors on fragile graphs but remain stable on redundant graphs. This framing highlights a concrete mechanism: chains rely on precise adjacency, whereas grids admit redundant paths that dampen perturbation effects. The paper therefore emphasizes empirical circumstances under which graph priors help or hurt, rather than asserting universal gains.

\paragraph{Contributions.}
The main contributions of this work are as follows.
\begin{itemize}[leftmargin=*]
    \item We map a controlled misspecification regime on chain and grid graphs, showing that weak regularization is broadly robust while stronger regularization produces topology-dependent degradation on fragile graphs.
    \item We quantify an \emph{illusory smoothness} effect in which smoothness under the imposed prior can improve even as true-support recovery worsens.
    \item We identify a graph-aligned positive case in which regularization improves structural coherence without sacrificing recovery, and include a small TCGA-BRCA sanity check.
...
We evaluate chain and grid graph families under controlled misspecification. Synthetic data and true supports are generated from the clean graph, while corruption is applied only to the \emph{prior} graph used by the method. We consider two corruption types---edge deletion and random rewiring---with a fixed corruption level $\alpha=0.2$ (fraction of $|E|$). We sweep $\lambda_2 \in \{0, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0\}$ with seeds $\{0,1,2\}$ for sweep-based results; the main chain baseline table reports seeds $0$--$4$ to anchor method comparison. The comparison set includes PCA, A-ManPG, GraphOnlyPCA ($\lambda_1=0$), a non-manifold graph+sparsity baseline (GraphSparseNoOrth), SparseNoGraph, and the Proposed method. SparseNoGraph is the in-family ablation with $\lambda_2=0$, while A-ManPG is the minimal external sparse orthogonal baseline without graph coupling. Shared evaluation metrics only are reported: support recovery, graph smoothness on the true graph, and shared explained variance.
...
\paragraph{Overview.}
We report synthetic results using shared evaluation metrics across all methods: union support F1, graph smoothness norm on the true graph, and shared explained variance (after QR orthonormalization). Shared explained variance is an evaluation metric and is not the training objective for any method. We do not compare raw objective values across methods because they optimize different objectives. Results are presented for chain and grid graphs across sweep seeds (0--2), with a chain baseline table across seeds 0--4 to anchor method comparison.

\subsection{Chain \texorpdfstring{$\lambda_2$}{lambda2} Sweep}
Figure~\ref{fig:chain-lambda2-sweep} reports the $\lambda_2$ sweep on chain graphs under clean, delete, and rewire priors. At low $\lambda_2$, curves overlap, indicating robustness when the graph prior is weak. At higher $\lambda_2$, the chain becomes misspecification-sensitive: clean retains higher recovery, deletion degrades moderately, and rewire degrades most, suggesting that in fragile topologies the regularization term quickly dominates the data-driven covariance signal. This establishes the chain mechanism: strong regularization amplifies errors in local adjacency.
\begin{figure}[t]
\centering
...
\subsection{Baseline Method Comparison on Chain (Seeds 0--4)}
Dense baselines (PCA, GraphOnlyPCA) fail on support recovery, while sparse baselines recover well; the non-manifold sparse+graph baseline remains weaker in recovery and variance. Graph regularization improves smoothness relative to the purely sparse variant, and explicit orthogonality keeps the full method competitive in recovery while preserving smoother structure. Shared explained variance is similar across the sparse methods and lower than PCA, as expected for sparse baselines. This establishes a clean baseline comparison before the regime analysis.
\begin{table}[t]
\centering
\scriptsize
...
\subsection{Chain vs.\ Grid Misspecification Comparison}
Figure~\ref{fig:phase2-compare} contrasts chain and grid under the same corruption and $\lambda_2$ sweep. The chain exhibits clear degradation under rewiring at moderate-to-high $\lambda_2$, while grid graphs remain largely invariant under the same perturbations. Quantitatively, the clean--rewire gap in support F1 at $\lambda_2=0.5$ and $1.0$ is $+0.058$ and $+0.076$ for the chain. For the grid, the corresponding gaps are $-0.041$ and $-0.028$ (positive = clean better, negative = rewire better). The small sign changes on the grid indicate that misspecification effects are weak and inconsistent in sign, consistent with robustness rather than genuine benefit.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{phase2/compare/blockAB_chain_grid_lambda2_sweep.png}
\caption{Chain vs.\ grid comparison under corruption. The chain shows degradation under rewiring at moderate-to-high $\lambda_2$, while the grid remains largely invariant.}
\label{fig:phase2-compare}
\end{figure}

\paragraph{Illusory smoothness (quantified).}
At $\lambda_2=0.5$ and $\alpha=0.2$ on the chain, rewiring reduces the imposed-graph Laplacian energy from $1.916$ to $1.599$ ($\Delta=-0.317$) while the true-graph Laplacian energy worsens (mean increases by $0.323$) and support F1 drops by $0.058$ (mean over seeds $0$--$2$), illustrating that smoothness under a corrupted prior can improve even as fidelity to the true graph and recovery degrade.

\subsection{Corruption Phase Diagram}
Figure~\ref{fig:phase2-alpha} reports a phase diagram over $(\lambda_2,\alpha)$ for chain graphs, with separate panels for deletion and rewiring. The color scale shows absolute support F1 on the true graph. Misspecification effects strengthen with increasing $\alpha$, and the most pronounced degradation occurs when both $\lambda_2$ and $\alpha$ are large; Appendix~\ref{app:topology} reports SBM and kNN misspecification results that follow the same topology-dependent pattern.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{phase2/compare/phase2_chain_alpha_lambda2_phase_diagram.png}
\caption{Phase diagram on chain graphs. Absolute support F1 on the true graph as a function of $(\lambda_2,\alpha)$ for deletion (left) and rewiring (right). Sensitivity grows as both $\lambda_2$ and $\alpha$ increase, delimiting a robust low-$\lambda_2$ region.}
\label{fig:phase2-alpha}
\end{figure}

\subsection{Graph-Aligned Positive Case}
Figure~\ref{fig:graph-aligned-sweep} summarizes the graph-aligned chain experiment. In this aligned regime, moderate $\lambda_2$ preserves recovery while improving structural coherence, illustrating that graph regularization can help when the prior is matched to the signal.
\begin{figure}[t]
\centering
...
\subsection{Real-Data Sanity Check (TCGA-BRCA)}
We run a $\lambda_2$ sweep on TCGA-BRCA expression with a STRING PPI graph using three random initializations and report smoothness and shared explained variance (Figure~\ref{fig:realdata}). We observe that moderate $\lambda_2$ yields stable variance--smoothness tradeoffs, while larger $\lambda_2$ leads to more aggressive smoothing with diminishing variance gains, consistent with the synthetic regime behavior; this qualitative stability under moderate $\lambda_2$ is consistent with the broader topology-dependent picture observed in the synthetic experiments.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{phase2/realdata/realdata_lambda2_sweep.png}
\caption{Real-data $\lambda_2$ sweep on TCGA-BRCA with STRING graph. Increasing $\lambda_2$ produces progressively smoother solutions, while explained variance improves up to a moderate regime and then plateaus with slight decline at higher $\lambda_2$.}
\label{fig:realdata}
\end{figure}
...
These experiments establish a focused empirical message: graph regularization is not universally helpful, and its reliability depends on both regularization strength and topology. On chain graphs, strong regularization amplifies misspecification effects, especially under rewiring; on grid graphs, performance is largely invariant under the same perturbations. We also observe an illusory smoothness effect: smoothness measured under the imposed (possibly corrupted) prior can improve even as true-support recovery worsens. In the graph-aligned chain setting with disjoint contiguous supports and scattered high-variance decoys, moderate regularization preserves recovery while improving structural coherence, indicating a setting where the graph prior is genuinely helpful.

A simple mechanism explains this pattern. Chains depend on precise adjacency: rewiring breaks the local relationships that Laplacian smoothness is meant to encode. Grids have redundant paths; redundancy flattens the effect of perturbations and reduces sensitivity to misspecification.

The main limitation of the current evidence is its synthetic emphasis and narrow topology coverage in the main text. We include a small TCGA-BRCA sanity check and move broader topology and real-data evidence to the appendix. Accordingly, we interpret the paper as a controlled characterization of reliability under misspecification rather than a comprehensive survey of graph families. These results demonstrate that structural priors can help, but they also show that regularization strength must be tuned to topology and that smoothness under the imposed graph should not be conflated with structural fidelity.

\section{Conclusion}
\label{sec:conclusion}

Graph regularization is not universally helpful. Its reliability depends on regularization strength and topology, and smoothness under the imposed graph can be misleading under misspecification. In aligned settings, however, graph priors can improve structural coherence without sacrificing recovery. These findings support a modest, defensible claim: use graph regularization with topology-aware tuning and interpret smoothness gains cautiously, especially when the graph is sparse or known to be noisy. More broadly, the results suggest that smoothness under an imposed graph should be treated as a topology-sensitive diagnostic rather than as evidence of structural fidelity.

\appendix

\section{Additional Topology Results}
\label{app:topology}
...
\subsection{SBM \texorpdfstring{$\lambda_2$}{lambda2} Sweep}
Sweeping $\lambda_2 \in \{0.0, 0.05, 0.1, 0.2, 0.5\}$ across seeds (0--2) reveals a stable recovery--smoothness tradeoff on SBM: stronger regularization steadily reduces support F1 while improving smoothness; light regularization improves smoothness with limited recovery loss. A-ManPG provides a fixed sparse baseline reference line.
\begin{figure}[t]
\centering
...
\subsection{Broader Misspecification Topologies (SBM, kNN)}
We extend the misspecification analysis to SBM and kNN topologies. Figure~\ref{fig:corruption-phase} presents phase diagrams over $(\lambda_2,\alpha)$ for different graph families, and Figure~\ref{fig:corruption-summary} summarizes a representative high-regularization setting. The same topology-dependent pattern holds: fragile graphs degrade under corruption, while redundant graphs are substantially more stable and can even show slight improvements or near-invariant performance.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{corruption_phase_panels.png}
\caption{Topology-dependent sensitivity under misspecification. Each panel shows the change in support F1 relative to the clean graph (same $\lambda_2$) as a function of $\lambda_2$ and corruption level $\alpha$, averaged over seeds.}
\label{fig:corruption-phase}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{corruption_delta_summary.png}
\caption{Change relative to the clean graph at a representative high-regularization setting ($\lambda_2 = 0.5, \alpha = 0.4$). Chain shows clear degradation, whereas grid, SBM, and kNN exhibit little degradation or slight improvement or near-invariant performance. Bars show mean $\Delta$F1 over seeds; error bars indicate standard error.}
\label{fig:corruption-summary}
\end{figure}

\section{Additional Real-Data Results}
\label{app:realdata}

\subsection{Unified Real-Data Sweeps}
We report unified $\lambda_2$ sweeps on TCGA-BRCA (STRING), MNIST (grid), and S\&P500 (correlation) to assess how the graph prior affects variance--smoothness tradeoffs in realistic settings. The trends are consistent with the topology-dependent behavior observed in synthetic experiments: MNIST (grid-like) exhibits stable performance across $\lambda_2$, TCGA-BRCA (modular) shows moderate sensitivity, and S\&P500 (noisy correlation graph) is more sensitive at higher $\lambda_2$.
\begin{figure}[t]
\centering
...
\subsection{TCGA Stability and Graph Coherence}
We assess stability across seeds and graph-coherence of selected supports against both uniform and degree-matched nulls at $\lambda_2=0.1$.
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../figures/realdata/tcga_stability_coherence.png}
\caption{TCGA-BRCA deep dive at $\lambda_2=0.1$ (Proposed). Left: seed-to-seed stability of the learned supports and subspaces. Right: graph-coherence diagnostics for the selected supports compared to uniform and degree-matched random gene sets.}
\label{fig:tcga-stability-coherence}
\end{figure}

\section{Implementation Details and Extended Tables}
\label{app:implementation}

\subsection{Graph-Aligned Chain Summary Table}
\begin{table}[t]
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
...
We use fixed step sizes ($\eta_A=0.05$) and the Lipschitz-based $\eta_B$ with QR retraction for the $A$-step. The stopping criteria and tolerances are as specified in Section~\ref{sec:theory}; all runs share the same thresholds to avoid tuning against evaluation metrics. Runtime warnings are logged and outputs are checked for finite metrics before inclusion in the summaries.
